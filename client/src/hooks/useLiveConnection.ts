...
export function useLiveConnection() {
  ...
  const setupAudioPlayback = useCallback(async () => {
    ...
    try {
      console.log("Loading audio player worklet from:", PLAYER_WORKLET_PATH);
  ...
  const connect = useCallback(
    async (
      ...
      try {
        ...
        if (source === 'text') {
          // Text-only mode: request mic for audio context, but don't use video/screen
          try {
            console.log("Text-only mode: Requesting microphone for audio context");
            stream = await navigator.mediaDevices.getUserMedia({
              audio: true,
              video: false,
            });
          } catch (e) {
            console.warn("Could not get microphone in text-only mode:", e);
            stream = null;
          }
        ...
        ws.onmessage = (event) => {
          ...
          for (const part of agentEvent.parts) {
            if (part.type === "audio/pcm") {
              console.log("Received audio/pcm data, attempting to play");
              const audioDataBytes = base64ToArray(part.data);
              if (audioPlayerNodeRef.current) {
                console.log("Audio player node exists, sending data");
                audioPlayerNodeRef.current?.port.postMessage(
                  { type: 'audio_data', buffer: audioDataBytes.buffer },
                  [audioDataBytes.buffer]
                );
              } else {
                console.warn("Audio player node is null - audio playback not initialized");
              }
            }
          }
          ...
          // Persist the agent's final spoken reply into the event log when
          // the turn completes. The server sends the transcript via
          // output_transcription (never as a text part), so we mirror it
          // here on turn_complete to match the working project.
          if (agentEvent.turn_complete && agentEvent.output_transcription?.text) {
